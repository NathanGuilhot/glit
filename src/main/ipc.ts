...
export function setupIpcHandlers(getWindow: () => BrowserWindow | null): void {
  ...
  ipcMain.handle('worktree:list', async (_event, repoPath: string) => {
    ...
    return listWorktreesWithDiff(repoPath)
  })

  ipcMain.handle('worktree:delete', async (_event, options: DeleteWorktreeOptions) => {
    log.info(`Deleting worktree: ${options.worktreePath}, force: ${options.force}, deleteFiles: ${options.deleteFiles}`)
    return deleteWorktree(options.repoPath, options)
  })

  ipcMain.handle('worktree:create', async (_event, options: CreateWorktreeOptions) => {
    const { repoPath, branchName, createNewBranch = false } = options
    ...
    try {
      ...
      const result = await createWorktree(repoPath, options, signal)

      // Load and run setup
      if (!signal.aborted) {
        try {
          sendWindowEvent(getWindow, 'create:progress', { step: 'packages', message: 'Running setup...' })
          await runSetupSteps(repoPath, result.worktreePath)
        } catch (e) {
          if (!signal.aborted) {
      ...
      return {
        success: true,
        worktree: { path: result.worktreePath, displayPath: shortenPathForDisplay(result.worktreePath), branch: branchName, isBare: false, isLocked: false },
      }
    } catch (error) {
      activeCreateController = null
      if (signal.aborted) {
  ...
  ipcMain.handle('worktree:sync', async (_event, worktreePath: string) => {
    ...
    return syncWorktree(worktreePath)
  })
  ...
  ipcMain.handle('branch:rebaseOnto', async (_event, repoPath: string, mainBranch: string) => {
    ...
    return rebaseOnto(repoPath, mainBranch)
  })
  ...
  ipcMain.handle('worktree:getMergedBranches', async (_event, repoPath: string, baseBranch: string) => {
    ...
    try {
      return await getMergedBranches(repoPath, baseBranch)
    } catch (error) {
