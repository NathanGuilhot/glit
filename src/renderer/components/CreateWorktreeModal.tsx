...
export default function CreateWorktreeModal({
  ...
  useEffect(() => {
    const loadData = async () => {
      const [branchList, config] = await Promise.all([
        window.glit.branch.list(repoPath).catch(() => [] as BranchInfo[]),
        window.glit.setup.preview(repoPath).catch(() => null),
      ])
  ...
  const progressPercent = progress
    ? (getStepIndex(progress.step) / (steps.length - 1)) * 100
    : 0
          ...
          {isWorking || isDone || isError ? (
            ...
            <VStack spacing={4} py={2}>
              <Progress
                ...
                w="full"
                colorScheme={isError ? 'red' : isDone ? 'green' : 'brand'}
                borderRadius="full"
                hasStripe={isWorking}
